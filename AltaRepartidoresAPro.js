...
export default class AltaRepartidoresAPro extends Component {
  constructor(props) {
    super(props);
    this.state = {
        //Variables
  ...
  render() {
    const btnAlta = () => {
        ...
        xhttp.onreadystatechange = function () {
            if (this.readyState == 4 && this.status == 200) {
                if (xhttp.responseText == '1') {
                    alert('Los datos del repartidor se dieron de alta');
                    btnRegresar();
                } 
                else {
        ...
        xhttp.open('GET', 'http://appudeg.xp3.biz/Proyecto/altaRepartidoresPro.php?nombre=' + this.state.name + '&imagen=' + this.state.url, true);
        xhttp.send();
    };
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.container2}>
            <Text style={styles.title2}> ALTA DE REPARTIDORES </Text>
        </View>
        <Text style={styles.text}> Llena los siguientes campos para dar de alta a un repartidor </Text>
        <TextInput
            style={styles.input}

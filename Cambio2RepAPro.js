...
export default class Cambio2RepAPro extends Component {
  constructor(props) {
    super(props);
    this.state = {
        //Variables
  ...
  render() {
        ...
        xhttp.onreadystatechange = function () {
          if (this.readyState == 4 && this.status == 200) {
            ...
            if (xhttp.responseText == '1') {
              alert('Se actualizó la información del repartidor de manera correcta');
              btnRegresar();
            } else {
              alert('ERROR: no se actualizó la información del repartidor');
              btnRegresar();
            }
          }
        };
        xhttp.open('GET', 'http://appudeg.xp3.biz/Proyecto/cambioRepartidoresPro.php?id=' + this.props.route.params.id + '&nombre=' + this.state.name + '&imagen=' + this.state.url, true);
        xhttp.send();
    }
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.text}> Introduce la información que deseas modificar del repartidor </Text>
        <Text style={styles.menu}> Información del repartidor con ID {this.props.route.params.id} </Text>
        ...
        <TextInput
            style={styles.input}
            placeholder="Introduce el nombre del repartidor"
